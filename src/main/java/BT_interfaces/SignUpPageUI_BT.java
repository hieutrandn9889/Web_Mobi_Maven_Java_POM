package BT_interfaces;

public class SignUpPageUI_BT {
	public static final String SIGN_UP_BUTTON = "//*[@text='Sign Up']";
	public static final String LOGIN_BUTTON = "//*[@text='Login']";
	public static final String FB_BUTTON = "(//android.view.ViewGroup)[7]";
	public static final String GOOGLE_PLUS_BUTTON = "(//android.view.ViewGroup)[8]";
	public static final String EMAIL_TEXTBOX = "//*[@text='Enter email here']";
	public static final String PASSWORD_TEXTBOX = "//*[@text='Enter password']";
	public static final String CONFIRM_PASSWORD_TEXTBOX = "//*[@text='Confirm password']";
	
}
